...
- ShapeUp ya tiene `entrenarState.ts`, `metricas.ts` y `filtros.ts` con sus tests, y
  `elegibilidad.ts` implementado; solo `recomendaciones.ts` queda pendiente (futuro).
...
- **PWA** con `vite-plugin-pwa` (instalable, offline). `useInstallPrompt` para el "agregar a
  pantalla de inicio".
- Build: **`tsc -b && vite build`** (typecheck + bundle). *Resuelto desde E1:* el scaffold
  arrancó con `vite build` solo para destrabar el deploy; ya volvió a `tsc -b && vite build`
  para no publicar con errores de tipo.
