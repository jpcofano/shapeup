# Prompt 45 — S0 · Documentar el plan de integración de salud en `CLAUDE.md`

> **Solo documentación** (no toques código de la app ni tests). Origen: decisión del owner
> (2026-07-04). Arranca la **serie S (Salud)**: conectar la última milla del match biométrico,
> simplificar la pantalla de Salud y sumar el motor de recomendaciones con rutinas nuevas.
> Antes de escribir una línea de código, dejamos el plan y las decisiones **escritas en un
> `CLAUDE.md` en la raíz del repo**, que además va a servir como memoria de proyecto para
> Claude Code en todas las sesiones futuras. Castellano voseo.
>
> **Hallazgo que motiva la serie S (verificado contra el repo el 2026-07-04):** el pipeline
> del match biométrico existe completo pero está **cortado en la última milla**:
> - `samsungZip.ts` (nivel "biometrico") ya extrae `shapeUpCustomId` y curvas `liveData` ✔
> - `lib/matchBiometrico.ts` (`elegirSesionSamsung`, `construirBiometriaSesion`,
>   `enriquecerSerie`) está escrito y testeado ✔
> - `Historial.biometria` existe en el modelo y `HistorialDetalle.tsx` ya lo renderiza ✔
> - **PERO** `handleZip` en `Salud.tsx` descarta `result.shapeUpCustomId` y `result.liveData`,
>   `matchBiometrico.ts` no se importa desde ningún lado de la app, y no existe función en
>   `data/historial.ts` para persistir la biometría de un Historial. Además,
>   `getMetricasSalud` (HRV, FC reposo, VO2max, estrés, pasos…) **nunca se usa en la UI**.

## (1) Creá `CLAUDE.md` en la raíz del repo

Con exactamente este contenido (ajustá solo si algún dato del repo lo contradice):
...
## (2) Sincronizá `docs/ESTADO-DEL-PROYECTO.md`

Está desactualizado respecto del repo:
- P23 y P24 figuran como "pendientes de aplicar" pero **ya están aplicados** (el código
  tiene `inicioMs/finMs` en `SerieRegistro`, importador zip-first con nivel biométrico, y
  la suite creció a ~231 tests). Corregí la sección de prompts y el conteo de tests real
  (corré `npx vitest run` y poné el número que dé).
- Agregá la serie S al final de "Pendientes" con referencia: *"Plan y decisiones: ver
  `CLAUDE.md` (serie S, ADRs #019–#022)"*. No dupliques el plan ahí; una línea y el link.

## (3) Verificación

- `CLAUDE.md` existe en la raíz, renderiza bien en GitHub (chequeá el fence de código).
- `docs/ESTADO-DEL-PROYECTO.md` sin referencias a P23/P24 como pendientes.
- `npx tsc -b` limpio y `npx vitest run` verde (no debería cambiar nada: este prompt no
  toca código).
- Commit sugerido: `docs: CLAUDE.md con plan serie S (salud) + ADRs #019-#022; sync estado`.
